...
class AlfrescoApi {
    ...
    setConfig(config) {
        ...
        if (this.isOauthConfiguration()) {
            this.oauth2Auth = oauth2Auth_1.Oauth2Auth.getInstance(this.config);
    ...
    clientsFactory() {
        if (!this.contentPrivateClient) {
            this.contentPrivateClient = new contentClient_1.ContentClient(this.config, '/api/-default-/private/alfresco/versions/1');
        }
        else {
            this.contentPrivateClient.setConfig(this.config, '/api/-default-/private/alfresco/versions/1');
        }
        if (!this.contentClient) {
            this.contentClient = new contentClient_1.ContentClient(this.config, '/api/-default-/public/alfresco/versions/1');
        }
        else {
            this.contentClient.setConfig(this.config, '/api/-default-/public/alfresco/versions/1');
        }
        if (!this.authClient) {
            this.authClient = new contentClient_1.ContentClient(this.config, '/api/-default-/public/authentication/versions/1');
        }
        else {
            this.authClient.setConfig(this.config, '/api/-default-/public/authentication/versions/1');
        }
        if (!this.searchClient) {
            this.searchClient = new contentClient_1.ContentClient(this.config, '/api/-default-/public/search/versions/1');
        }
        else {
        ...
        if (!this.gsClient) {
            this.gsClient = new contentClient_1.ContentClient(this.config, '/api/-default-/public/gs/versions/1');
        }
        else {
    ...
    exchangeTokenForAlfTicket() {
        this.oauth2Auth.once('token_issued', () => {
            if (this.config.provider === 'ALL' || this.config.provider === 'ECM') {
                let authContentApi = new authentication_api_1.AuthenticationApi(this);
                authContentApi.getTicket().then((ticketEntry) => {
